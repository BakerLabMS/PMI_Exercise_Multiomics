---
title: "PMI_Multiomics_dendrogram"
author: "Melanie Odenkirk"
output: html_document
---

This file includes the multiomic circular dendrogram output used to construct Figure 5 of "From Prevention to Disease Perturbations: A Multi-Omic Assessment of Exercise and Myocardial Infarctions" by Odenkirk et al.
...
exer_fn <- c("PMI_multiomics_results")

fn <- exer_fn[1]
sig.df <- data.frame(matrix(nrow = 330, ncol = length(exer_fn)))
for( i in 1 : length(exer_fn)) {
  fn <- exer_fn[i]
...
#load all desc.
cmpds <- read.csv("PMI_node_color_multiomics.csv", header= TRUE)
...
cols10 =c("Car" = "olivedrab3","CE" = "purple3", "Cer" = "black", "DG" = "gold3", "Met" = "darkcyan" ,"MG"= "red", "LPC" = "seagreen2", "PC" = "violetred3", "PC O-" = "turquoise2", "PC P-" = "darkorange", "LPE" = "blue", "PE" = "forestgreen", "PE P-" = "salmon", "PI" = "dodgerblue1","SM" = "ghostwhite","TG" = "navy",  "PE O-" = "plum1")
...
t4 <- t4 %<+% cmpds +
 geom_tippoint(aes(color=HG), size=4, alpha = 0.5, shape = 16)+
...
t4 <- gheatmap(t4, to_plot3[1], offset = 0, width = 0.12, colnames = F)  +
  scale_fill_gradient2(low = rampCol1, high = rampCol2, mid = "black",  midpoint = 0)
